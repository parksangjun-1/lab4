 ...
 * 2. find requests whose rdy cycles are expired
 * 3. performs a cache lookup in the "cache base" after the intrinsic access time
 * 4. on a cache hit, forward the request to the prev's fill_queue or the processor depending on the cache level.
 * 5. on a cache miss, put the current requests into out_queue
 * 6. pop done reqs from in_queue
 */
 ...
 * Iterate the reqs in the queue, fill or access, and pop done requests
 ...
 * iterate, find expired reqs, write-back or fill, and pop done reqs.
 * fill: accessed from the lower level cache/memory (according to hierarchy level), and handle back_inval and writeback caused by that miss
 */
 ...
 * The function basically moves the requests from wb_queue to out_queue, and pop moved reqs from fill queue.
 * CURRENT: There is no limit on the number of requests we can process in a cycle.
 */
